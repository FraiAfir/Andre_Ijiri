...
// 1: Testa se a função criarParametro() retorna um ponteiro não NULL, indicando que o objeto de parâmetros foi criado com sucesso
void test_CriarParametro_DeveRetornarNaoNULL(void){
    // 1: Cria um objeto de parâmetros usando a função criarParametro() para ser usado nos testes subsequentes
    param_teste = criarParametro();

    // 2: Verifica se o ponteiro retornado pela função criarParametro() é diferente de NULL, indicando que o objeto de parâmetros foi criado com sucesso
    TEST_ASSERT_NOT_NULL(param_teste);
    printf("\n");
}

// 2: Testa se a função processarParametros() retorna 0, indicando que os parâmetros da linha de comando foram processados com sucesso
void test_ProcessarParametros_DeveRetornarZero(void){
    // 1: Cria um objeto de parâmetros usando a função criarParametro() para ser usado nos testes subsequentes
    param_teste = criarParametro();

    // 2: Cria um array de argumentos simulando a linha de comando para testar a função processarParametros()
    char* argv[] = {"programa", "-f", "arquivo.geo", "-o", "dirSaida"};
    int argc = sizeof(argv) / sizeof(argv[0]);

    // 3: Chama a função processarParametros() para processar os parâmetros da linha de comando
    int resultado = processarParametros(param_teste, argc, argv);

    // 4: Verifica se o resultado retornado pela função processarParametros() é igual a 0, indicando que os parâmetros foram processados com sucesso
    TEST_ASSERT_EQUAL_INT(0, resultado);
    printf("\n");
}

// 3: Testa se a função freeParametros() retorna 0, indicando que a memória alocada para o objeto de parâmetros foi liberada com sucesso
void test_LiberarMemoria_DeveRetornarZero(void){
    // 1: Cria um objeto de parâmetros usando a função criarParametro() para ser usado nos testes subsequentes
    param_teste = criarParametro();

    // 2: Chama a função freeParametros() para liberar a memória alocada para o objeto de parâmetros
    int resultado = freeParametros(param_teste);

    // 3: Verifica se o resultado retornado pela função freeParametros() é igual a 0, indicando que a memória foi liberada com sucesso
    TEST_ASSERT_EQUAL_INT(0, resultado);
